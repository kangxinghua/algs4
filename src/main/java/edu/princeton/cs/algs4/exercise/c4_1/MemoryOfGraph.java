...
public class MemoryOfGraph {
	...
	// create a uniformly random k-regular graph on V vertices (not necessarily
	// simple)
	...
	// min memory of GraphX - assuming adjacency lists use resizing array
	// representation
	// and uses primitive type int (so don't need to worry about caching
	// Integers)
	public static void minMemoryOfGraphX() {
		...
		long[] memory = new long[N];
		for (int i = 0; i < N; i++) {
			int n = 2 + 2 * StdRandom.uniform(500); // a multiple of 2
			int k = 1 << (1 + StdRandom.uniform(6)); // a power of 2 >= 2 since
														// 2 is min size of
														// resizing array
			GraphX G = regular(n, k);
	...
	public static void maxMemoryOfGraphX() {
		...
		for (int i = 0; i < N; i++) {
			int n = 2 + 2 * StdRandom.uniform(500); // a multiple of 2
			int k = 1 + (1 << (1 + StdRandom.uniform(6))); // one more than a
															// power of 2 >= 2
															// since 2 is min
															// size of resizing
															// array
